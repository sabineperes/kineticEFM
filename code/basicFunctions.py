...
def km_sorting(excelfile,model):
    ...
    km_matrix =  np.full((len(model.metabolites),len(model.reactions)),0.1) ##creation of a model matrix filled by default to 0.1
...
def produit2(x, l, km, model, j):
    ...
    for i, item in enumerate(l):
        if km[int(item), j] != 0 :
                if ((int(item) != k_H) & (int(item) != k_Hext)) :
                    res *= (x[int(item)] / km[int(item), j]) ** abs(st[int(item), j])
        else:
            raise TypeError('Should no longer contain any null value.')
    return res    
